        ...
        if not self.args.knowledge_distillation:
            ...
            if 'pos_scores' not in temp_dataset.column_names or 'neg_scores' not in temp_dataset.column_names:
                raise ValueError(
                    ...
                    f"which is required for knowledge distillation."
                )
        return temp_dataset
    ...
    def __getitem__(self, item):
        ...
        if self.args.knowledge_distillation:
            ...
            if not all(isinstance(score, (int, float)) for score in teacher_scores):
                raise ValueError("pos_score or neg_score must be numeric")
        else:
            teacher_scores = None
...
class PairBatchDataset(PairDataset):
    def __init__(
        self,
        args,
    ...
    @staticmethod
    def _get_file_batch_size(temp_dataset, default_batch_size: int):
...
class PairCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        if self.sub_batch_size is None or self.sub_batch_size <= 0:
            q_collated = self.tokenizer.pad(
                queries_inputs, padding=self.padding, max_length=self.query_max_len,
                pad_to_multiple_of=self.pad_to_multiple_of, return_tensors=self.return_tensors
            )
            d_collated = self.tokenizer.pad(
                passages_inputs, padding=self.padding, max_length=self.passage_max_len,
                pad_to_multiple_of=self.pad_to_multiple_of, return_tensors=self.return_tensors
            )
        else:
            q_collated = self._pad_sub_batches(queries_inputs, self.query_max_len)
            d_collated = self._pad_sub_batches(passages_inputs, self.passage_max_len)
    ...
    def _pad_sub_batches(self, inputs, max_length):
        batch_size = self.sub_batch_size
        result = []
        for i in range(0, len(inputs['attention_mask']), batch_size):
            end = min(len(inputs['attention_mask']), i + batch_size)
            sub_features = {k: v[i:end] for k, v in inputs.items()}
            result.append(self.tokenizer.pad(
                sub_features, padding=self.padding, max_length=max_length,
                pad_to_multiple_of=self.pad_to_multiple_of, return_tensors=self.return_tensors
            ))
        return result
...
@dataclass
class BatchCollator(DataCollatorWithPadding):
    query_max_len: int = 32
    passage_max_len: int = 128
    sub_batch_size: int = -1
    default_no_in_batch_neg_flag: bool = False
...
class EpochRefreshCallback(TrainerCallback):
    def __init__(self, train_dataset: PairBatchDataset):
