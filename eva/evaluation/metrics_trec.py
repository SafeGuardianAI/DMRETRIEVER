...
metrics_trec.py
────────────────────────────────────────────────────────────────────────────
Evaluate trec_covid (qrels-based: query_id ↔ doc_id ↦ score), computing:
  NDCG@10 / MRR@10 / Recall@10 (zero-fill and ignore-list variants)
Output:
  performance/<parent>/<ckpt_name>/, aligned with the main metrics layout (for skip_done)
"""
...
_TREC_ROOT  = Path(RAW_DATA_DIR)
_QRELS_FP   = _TREC_ROOT / "trec_covid_qrels.json"
...
def _find_trec_label_file(label_dir: Path) -> Path:
    """
    Find the trec label_pool file (*_label_pool.json written by exact.py, containing doc_ids).
    """
    cands = sorted(label_dir.glob("*_label_pool.json"))
    if not cands:
        raise FileNotFoundError(f"no label_pool json under: {label_dir}")
    # naming compatibility: prefer files containing 'trec'; otherwise use the single file
    for fp in cands:
        if "trec" in fp.stem:
            return fp
    return cands[0]

def calc_metrics_trec(model: str, parent: str) -> Path:
    """
    Read trec_covid label_pool and qrels, compute metrics, and write output to
    ...
    lab_fp = _find_trec_label_file(label_dir)
    ...
    # per-event（trec_covid）/ per-task（QA）
    def _row(sums):
        return dict(
            model=slug,
            search_task="QA",
            event_type="trec_covid",
            num_queries=sums["nq"],
    ...
    LOGGER.info("saved trec metrics ➜ %s", out_dir)
    return out_dir

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser("Calculate TREC-COVID metrics for one checkpoint")
    ...
    calc_metrics_trec(args.model, args.parent)
